...
(() => {
  ...
  const DB_VERSION = 5; // 分片新增第 8 字段英英释义（enDefs），bump 清掉旧格式分片缓存
  ...
  const BIG_SPLIT_PARTS = { c: 2, p: 2, s: 2 }; // 大分片 → 子文件份数（与数据仓拆分约定一致）
...
async function fetchShardJson(letter, timeoutMs, sessionSignal, onStage) {
  ...
  /**
   * @returns {object|null} {translation, phonetic, pos, tags, fields, collins, oxford, enDefs}
   */
  async function localLookup(word, sessionSignal) {
    ...
    try {
      ...
      // entry = [translation, phonetic, pos, tags[], fields[], collins, oxford, enDefs?]
      const result = {
        translation: entry[0] || '',
        phonetic: entry[1] || '',
        pos: entry[2] || '',
        ...
        collins: entry[5] || 0,
        oxford: entry[6] || 0,
        enDefs: typeof entry[7] === 'string' ? entry[7] : '',
      };
      return result;
    } catch (e) {
      return null;
  ...
  // 释义请求 8s 超时兜底：dictionaryapi.dev 经常宕机/国内超时，
  // 现在它只是「增强源」（例句/同义词/音标音频），本地英英释义已兜底，尽快落定
   ...
   * @param {Array|null} meanings API 释义（dictionaryapi.dev）
   * @param {boolean} ok API 是否成功
   * @param {string} localEnDefs 本地词库英英释义（ECDICT definition），API 失败时的静默回退
   */
  ...
  /**
   * 本地英英释义先行渲染：本地有释义立即填充（不等 API）；
   * 本地没有则保持"查询中…"占位，等在线路结果。
   */
  ...
  /**
   * 本地英英释义 HTML（"n. gloss\nv. gloss" 按词性分组，样式对齐 API 释义）
   */
  function buildLocalEnDefsHtml(enDefs) {
    const parts = ['<div class="ed-section-title">' + T.engDefsTitle + '</div>'];
    const lines = enDefs.split('\n').map(l => l.trim()).filter(Boolean).slice(0, 12);
    let curPos = '';
    let curDefs = [];
    const flush = () => {
      if (curDefs.length === 0) return;
      parts.push('<div class="ed-pos-group">');
      if (curPos) parts.push('<span class="ed-api-pos">' + escapeHtml(curPos) + '</span>');
      parts.push('<ol class="ed-defs">');
      for (const d of curDefs.slice(0, 5)) {
        parts.push('<li><span class="ed-def">' + escapeHtml(d) + '</span></li>');
      }
      parts.push('</ol></div>');
      curDefs = [];
    };
  ...
  function applyQueryCache(c, valid) {
    ...
    renderApiSlot(api ? api.meanings : null, !!api, local && local.enDefs);
      ...
      const launchLookup = () => {
        ...
        localLookup(ew, sessionCtrl.signal).then(ld => {
          ...
          if (ld) {
            ...
            result.enDefs = ld.enDefs;
            result.fromLocal = true;
            renderLocalSlot(ld);
            renderApiSlotLocal(ld.enDefs);
          } else {
        ...
        fetchDictAPI(ew, sessionCtrl.signal).then(ad => {
          ...
          renderApiSlot(ad ? ad.meanings : null, !!ad, result.enDefs);
          updateWordHeader(result);
          updateSourcesHtml(result);
        });
      };
